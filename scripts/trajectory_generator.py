...
class MultiAxisTrajectoryGenerator:
    ...
    def generatePositions(self, nsteps=100):
        """
        Generates only positions
...
class TrapezoidVelocity:
    ...
    def _copy_params(self, trajgen):
        self.start_pos = trajgen.start_pos[0]
        self.final_pos = trajgen.final_pos[0]
        self.T = trajgen.T
        self.ndof = trajgen.ndof
        self.X = [None] * self.ndof

    def solve(self, nsteps=100):
        self.V = (self.final_pos - self.start_pos) / self.T  # Average velocity
        self.peak_acceleration = 5  # Peak acceleration (can be adjusted)
        self.tb = self.V / self.peak_acceleration  # Time to reach peak velocity

        # Check if the motion is feasible within the given time
        if 2 * self.tb > self.T:
            # Adjust tb and V if the motion is infeasible
            self.tb = self.T / 2
            self.V = self.peak_acceleration * self.tb
    ...
    def generate(self, nsteps=200):
        self.t = np.linspace(0, self.T, nsteps)
        for i in range(self.ndof):  # iterate through all DOFs
            q, qd, qdd = [], [], []
            for t in self.t:  # iterate through time, t
                if t < self.tb:  # Acceleration phase
                    q.append(self.start_pos + (self.peak_acceleration * t**2) / 2)
                    qd.append(self.peak_acceleration * t)
                    qdd.append(self.peak_acceleration)
                elif t < self.T - self.tb:  # Constant velocity phase
                    q_acc_end = (
                        self.start_pos + (self.peak_acceleration * self.tb**2) / 2
                    )
                    q.append(q_acc_end + self.V * (t - self.tb))
                    qd.append(self.V)
                    qdd.append(0)
                    ...
                    q_dec_start = q_acc_end + self.V * (self.T - 2 * self.tb)
                    q.append(
                        q_dec_start
                        + self.V * (t - (self.T - self.tb))
                        - (self.peak_acceleration * (t - (self.T - self.tb)) ** 2) / 2
                    )
                    qd.append(
                        self.V - self.peak_acceleration * (t - (self.T - self.tb))
                    )
                    qdd.append(-self.peak_acceleration)
            self.X[i] = [q, qd, qdd]
        return self.X
